...
describe('CardLogin Component', () => {
  ...
  it('renders initial view with login button', () => {
    render(<CardLogin />);
    expect(screen.getByRole('button', { name: /iniciar con cuenta un bet/i })).toBeInTheDocument();
  });
  ...
  it('shows login form when login button is clicked', () => {
    render(<CardLogin />);
    const loginButton = screen.getByRole('button', { name: /iniciar con cuenta un bet/i });
