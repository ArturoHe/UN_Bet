...
export default function SlotMachine({ 
  ...
  const [reelSymbols, setReelSymbols] = useState<string[][]>(() => {
    ...
    return [0, 1, 2].map(() => {
      ...
      return Array.from({ length: VISIBLE_SYMBOLS }, (_, i) => 
        SYMBOLS[(startIdx + i) % TOTAL_SYMBOLS]
      );
    });
  });
  ...
  const animateReel = useCallback((reelIndex: number, targetIndex: number, duration: number, onComplete: () => void) => {
    ...
    const animate = (currentTime: number) => {
      ...
      if (currentTime - lastSymbolChange > currentInterval) {
        ...
        setReelSymbols(prev => {
          const newReels = [...prev];
          newReels[reelIndex] = Array.from({ length: VISIBLE_SYMBOLS }, (_, i) => 
            SYMBOLS[(currentIdx + i) % TOTAL_SYMBOLS]
          );
          return newReels;
        });
      }
      ...
      if (progress < 1) {
        ...
        setReelSymbols(prev => {
          const newReels = [...prev];
          newReels[reelIndex] = Array.from({ length: VISIBLE_SYMBOLS }, (_, i) => 
            SYMBOLS[(targetIndex + i) % TOTAL_SYMBOLS]
          );
          return newReels;
        });
  ...
  useEffect(() => {
    if (isSpinning && result && result.length === 3) {
      ...
      result.forEach((targetIndex, reelIndex) => {
        ...
        setTimeout(() => {
          animateReel(reelIndex, targetIndex, durations[reelIndex], handleReelComplete);
        }, delays[reelIndex]);
      });
    }
      ...
      <div className={styles.machine}>
        <div className={styles.frame}>
          ...
          <div className={styles.reelsContainer}>
            ...
            <div className={`${styles.winIndicator} ${winLine ? styles.winIndicatorActive : ''}`}>
              <span>➤</span>
            </div>
            
            {[0, 1, 2].map((reelIndex) => (
              <div key={reelIndex} className={styles.reelWrapper}>
                <div className={`${styles.reel} ${spinningReels[reelIndex] ? styles.spinning : ''}`}>
                  {reelSymbols[reelIndex].map((symbol, idx) => (
                    <div 
                      key={idx} 
                      className={`${styles.symbol} ${idx === 1 ? styles.symbolMiddle : ''}`}
                    >
                      {symbol}
                    </div>
                  ))}
            ...
            <div className={`${styles.winIndicatorRight} ${winLine ? styles.winIndicatorActive : ''}`}>
              <span>➤</span>
            </div>
          </div>
        ...
        {/* Botón de girar */}
        <button
          className={`${styles.spinButton} ${isCurrentlySpinning ? styles.spinButtonActive : ''}`}
